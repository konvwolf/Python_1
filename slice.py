...
def palindrome(d):    
    s = str(d)[0 : (len(str(d)) // 2)] == str(d)[-1 : -(len(str(d)) // 2) - 1 : -1]
    return s
...
def inside(s):
    ...
    s2 = s1[0:len(s1) // 2]
    s3 = s1[-len(s1) // 2:]
    return ' '.join(s2 + [s.split(' ')[0]] + [s.split(' ')[-1]] + s3)
...
def main():
    ...
    test(inside('a b c d e f g h i j k l'), 'b c d e f a l g h i j k')
